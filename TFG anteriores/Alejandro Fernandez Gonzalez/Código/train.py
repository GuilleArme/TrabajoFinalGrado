...
class TrainManager(object):
    ...
    def train(self):
        ...
        if self.cfg.validation.if_val_before_begin_train:
            initial_val_mae, initial_val_mse, initial_val_mape  = self.validate(
                self.val_loader, step=epoch)
            initial_tr_mae, initial_tr_mse, initial_val_mape  = self.validate(
        ...
        for epoch in tqdm(range(self.cfg.train.start_epoch, end_epoch)):
            ...
            if epoch % self.cfg.validation.validate_ckpt_every_epochs == 0:
                val_mae, val_mse, val_mape = self.validate(self.val_loader, step=epoch)
                tr_mae, tr_mse, tr_mape = self.validate(self.train_loader, step=epoch)
                self.tbx_wrtr.add_scalars(
                    'error_values/MAE',
...
def main():
    ...
    with SummaryWriter(additional_cfg['tbx_wrtr_dir'], comment="interval="+str(cfg.dataset.num_intervals)) as tbx_wrtr:
...
def train_main(dataset_rootdir, num_intervals, densmaps_gt_npz):
    """
    Create data loaders, the model instance, optimizer and TrainManager()
    object. Run the training process.
    """
    ...
    print('Entrenando con valores Cmax: '+ str(cfg.dataset.num_intervals) + ' y dmap: ' + str(cfg.dataset.densmaps_gt_npz ))
    train_loader, val_loader, test_loader = get_dataloaders(cfg, (1, 1, 0))
    interval_bounds, label2count_list = make_label2count_list(cfg)
    ...
    additional_cfg['tbx_wrtr_dir'] = os.getcwd()
    with SummaryWriter(additional_cfg['tbx_wrtr_dir'],
                       comment="interval=" + str(cfg.dataset.num_intervals)) as tbx_wrtr:
        additional_cfg['tbx_wrtr'] = tbx_wrtr
        trainer = TrainManager(
            model,
            optimizer,
